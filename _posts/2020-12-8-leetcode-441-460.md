...
And if the number of same distance points of current point is n, to construct boomerange, we need to pick two points, and the order is matter, so the count of current boomeranges 
equals to `n*(n-1)`.
